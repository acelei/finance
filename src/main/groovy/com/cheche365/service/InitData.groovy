...
'''
    ...
    private static final String settlementSum = "UPDATE settlement_# SET `10-全保费`=ifnull(`10-全保费`, 0),`11-净保费`=ifnull(`11-净保费`, 0),`14-手续费总额（报行内+报行外）(含税)`=ifnull(`14-手续费总额（报行内+报行外）(含税)`,0),`15-手续费总额（报行内+报行外）(不含税)`=ifnull(`15-手续费总额（报行内+报行外）(不含税)`,0),`19-手续费金额（含税）`=ifnull(`19-手续费金额（含税）`,0),`20-手续费金额（不含税）`=ifnull(`20-手续费金额（不含税）`,0),`23-收款金额`=ifnull(`23-收款金额`,0),`28-开票金额（含税）`=ifnull(`28-开票金额（含税）`,0),`29-20191231应收账款（含已开票和未开票）`=ifnull(`29-20191231应收账款（含已开票和未开票）`,0),`33-收款金额`=ifnull(`33-收款金额`,0),`36-开票金额（不含税）`=ifnull(`36-开票金额（不含税）`,0),`37-开票金额（含税）`=ifnull(`37-开票金额（含税）`,0),`38-尚未开票金额（不含税）`=ifnull(`38-尚未开票金额（不含税）`,0),`39-尚未开票金额（含税）`=ifnull(`39-尚未开票金额（含税）`,0),`42-佣金金额（已入账）`=ifnull(`42-佣金金额（已入账）`,0),`45-支付金额`=ifnull(`45-支付金额`,0),`46-未计提佣金（19年底尚未入帐）`=ifnull(`46-未计提佣金（19年底尚未入帐）`,0),sum_fee=ifnull(`14-手续费总额（报行内+报行外）(含税)`,0),sum_commission=0"
