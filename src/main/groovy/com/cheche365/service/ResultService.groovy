...
class ResultService extends DataCombine {
    ...
    String tableFinal = 'result_#_2_final'
...
select a.`id`,
       ...
       a.`1-序号`,
       a.`2-保代机构`,
       if(d.id is not null, d.`7-出单保险公司（明细至保险公司分支机构）`, if(e.id is not null, e.`7-出单保险公司（明细至保险公司分支机构）`, a.`7-出单保险公司（明细至保险公司分支机构）`)) as `3-出单保险代理机构（车车科技适用）`,
       if(d.id is not null, d.`7-出单保险公司（明细至保险公司分支机构）`, if(e.id is not null, e.`7-出单保险公司（明细至保险公司分支机构）`, a.`7-出单保险公司（明细至保险公司分支机构）`)) as `4-发票付款方（与发票一致）`,
       if(b.finance_id is not null, b.applicant, if(d.id is not null, d.`5-投保人名称`, if(e.id is not null, e.`5-投保人名称`, a.`5-投保人名称`)))    as '5-投保人名称',
       if(b.finance_id is not null, b.policy_no, if(d.id is not null, d.`6-保单单号`, if(e.id is not null, e.`6-保单单号`, a.`6-保单单号`)))      as '6-保单单号',
       if(d.id is not null, d.`7-出单保险公司（明细至保险公司分支机构）`, if(e.id is not null, e.`7-出单保险公司（明细至保险公司分支机构）`, a.`7-出单保险公司（明细至保险公司分支机构）`)) as `7-出单保险公司（明细至保险公司分支机构）`,
       if(b.finance_id is not null, if(b.insurance_type_id = 1, '交强险', '商业险'),
...
'''
    ...
    String fixSql = '''
update & set `13-手续费率`=`14-手续费总额（报行内+报行外）(含税)`/`11-净保费`,
                             ...
                             `3-出单保险代理机构（车车科技适用）`= if (s_id is null, `7-出单保险公司（明细至保险公司分支机构）`, ifnull(`3-出单保险代理机构（车车科技适用）`, `4-发票付款方（与发票一致）`)),
                             `4-发票付款方（与发票一致）`=if(s_id is null, `7-出单保险公司（明细至保险公司分支机构）`, `4-发票付款方（与发票一致）`)
'''
...
'''
    ...
    void fix(String type) {
        if (zTypes.contains(type)) {
            baseSql.executeUpdate(getFixSql2().replace("&",getTableFinal()).replace("#", type))
        } else {
            baseSql.executeUpdate(getFixSql().replace("&",getTableFinal()).replace("#", type))
        }
    }
...
//    List<String> head = ["id", "s_id", "d_id", "c_id", "source_file","1-序号", "2-保代机构", "3-出单保险代理机构（车车科技适用）", "4-发票付款方（与发票一致）", "5-投保人名称", "6-保单单号", "7-出单保险公司（明细至保险公司分支机构）", "8-险种名称", "9-保单出单日期", "10-全保费", "11-净保费", "12-手续费等级（对应点位台账）", "13-手续费率", "14-手续费总额（报行内+报行外）(含税)", "15-手续费总额（报行内+报行外）(不含税)", "16-收入入账月度", "17-凭证号", "18-手续费比例", "19-手续费金额（含税）", "20-手续费金额（不含税）", "21-回款月度", "22-凭证号", "23-收款金额", "24-开票单位", "25-开票日期", "26-手续费比例", "27-开票金额（不含税）", "28-开票金额（含税）", "29-20191231应收账款（含已开票和未开票）", "30-开票日期", "31-应收回款月度", "32-收款凭证号", "33-收款金额", "34-开票单位", "35-开票日期", "36-开票金额（不含税）", "37-开票金额（含税）", "38-尚未开票金额（不含税）", "39-尚未开票金额（含税）", "40-代理人名称", "41-佣金比例", "42-佣金金额（已入账）", "43-支付主体", "44-支付比例", "45-支付金额", "46-未计提佣金（19年底尚未入帐）"]
    ...
    List<String> sHead = ["1-序号", "10-全保费", "11-净保费", "13-手续费率", "14-手续费总额（报行内+报行外）(含税)", "15-手续费总额（报行内+报行外）(不含税)", "18-手续费比例", "19-手续费金额（含税）", "20-手续费金额（不含税）", "23-收款金额", "26-手续费比例", "27-开票金额（不含税）", "28-开票金额（含税）", "29-20191231应收账款（含已开票和未开票）", "33-收款金额", "36-开票金额（不含税）", "37-开票金额（含税）", "38-尚未开票金额（不含税）", "39-尚未开票金额（含税）", "41-佣金比例", "42-佣金金额（已入账）", "44-支付比例", "45-支付金额", "46-未计提佣金（19年底尚未入帐）"]
    ...
    File exportResult(String type, File targetFile) {
        ...
        if (rows.size() > 0) {
            ...
            file = ExcelUtil2.writeToExcel(head, rows)
            if (targetFile != null) {
