...
select group_concat(id) d_id,group_concat(s_id) as s_id,group_concat(c_id) as c_id,
       ...
       `8-险种名称`,
       `9-保单出单日期`,
       if(max(0+`10-全保费`)>0,max(0+`10-全保费`)+if(min(0+`10-全保费`)<0,min(0+`10-全保费`),0),min(0+`10-全保费`)) as `10-全保费`,
       if(max(0+`11-净保费`)>0,max(0+`11-净保费`)+if(min(0+`11-净保费`)<0,min(0+`11-净保费`),0),min(0+`11-净保费`)) as `11-净保费`,
       `12-手续费等级（对应点位台账）`,
       `13-手续费率`,
