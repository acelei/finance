...
select a.`id`,
       ...
       a.`3-出单保险代理机构（车车科技适用）`,
       a.`4-发票付款方（与发票一致）`,
       if(b.finance_id is not null, b.applicant, if(d.id is not null, d.`5-投保人名称`, a.`5-投保人名称`))    as '5-投保人名称',
       if(b.finance_id is not null, b.policy_no, if(d.id is not null, d.`6-保单单号`, a.`6-保单单号`))      as '6-保单单号',
       a.`7-出单保险公司（明细至保险公司分支机构）`,
       if(b.finance_id is not null, if(b.insurance_type_id = 1, '交强险', '商业险'),
          if(d.id is not null, d.`8-险种名称`, a.`8-险种名称`))                                             as '8-险种名称',
       if(b.finance_id is not null, b.order_date, if(d.id is not null, d.`9-保单出单日期`, a.`9-保单出单日期`)) as '9-保单出单日期',
       if(b.finance_id is not null, b.premium, if(d.id is not null, d.`10-全保费`, a.`10-全保费`))        as '10-全保费',
       if(b.finance_id is not null, b.premium / 1.06, if(d.id is not null, d.`11-净保费`, a.`11-净保费`)) as '11-净保费',
       a.`12-手续费等级（对应点位台账）`,
       a.`13-手续费率`,
...
from settlement_# a
         ...
         left join result_gross_margin_ref c on a.id = c.s_id and c.type = 2 and
                                                c.table_name in ('result_#_2', 'settlement_#', 'commission_#')
         left join commission_# d on d.id = c.c_id
'''
...
from commission_# a
         left join business_replace_ref b on a.id = b.finance_id and b.table_name = 'commission_#'
         left join result_gross_margin_ref c on a.id = c.c_id and c.type = 1 and
                                                c.table_name in ('result_#_2', 'settlement_#', 'commission_#')
         left join settlement_# d on d.id = c.s_id
'''
...
`42-佣金金额（已入账）`,`45-支付金额`,`46-未计提佣金（19年底尚未入帐）`, `保险公司`,`保险公司id`,`省`,`市`,sum_fee,sum_commission,gross_profit)
