...
class SBTCZLService {
    ...
    String sTableName = 'settlement_sbt_czl_gg'
    String cTableName = 'commission_sbt_czl_gg'
    ...
    void getCZL(String bdType) {
        ...
        baseSql.executeInsert("insert into ${sTableName} select ${sColumn} from settlement_${bdType} where source_file in ('settlement_sbt_czl_2')" as String)
        baseSql.executeInsert("insert into ${cTableName} select ${cColumn} from commission_${bdType} where source_file in ('commission_sbt_czl_2')" as String)
    }
...
from settlement_# a
         ...
         where a.handle_sign!=6 and a.source_file in ('settlement_sbt_czl_2')
'''
...
//            getCZL(it.target)
    ...
    @Test
    void reRun() {
        def rows = baseSql.rows("select `type` from table_type where org='保代-广管' and flag>0")
        rows.each {
            def type = it.type
